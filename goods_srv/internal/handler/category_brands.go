...
func (s *GoodsServer) CategoryBrandList(ctx context.Context, req *proto.CategoryBrandFilterRequest) (*proto.CategoryBrandListResponse, error) {
	var categoryBrands []db.GoodsCategoryBrand
	categoryBrandListResponse := proto.CategoryBrandListResponse{}

	var total int64
	global.DbClient.Model(&db.GoodsCategoryBrand{}).Count(&total)
	categoryBrandListResponse.Total = int32(total)

	global.DbClient.Preload("Category").Preload("Brands").Scopes(utils.Paginate(int(req.Pages), int(req.PagePerNums))).Find(&categoryBrands)
...
func (s *GoodsServer) GetCategoryBrandList(ctx context.Context, req *proto.CategoryInfoRequest) (*proto.BrandListResponse, error){
	...
	if result := global.DbClient.Find(&category, req.Id).First(&category); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "商品分类不存在")
	}

	var categoryBrands []db.GoodsCategoryBrand
	if result := global.DbClient.Preload("Brands").Where(&db.GoodsCategoryBrand{CategoryID: req.Id}).Find(&categoryBrands); result.RowsAffected > 0 {
...
func (s *GoodsServer) CreateCategoryBrand(ctx context.Context, req *proto.CategoryBrandRequest) (*proto.CategoryBrandResponse, error) {
	var category db.Category
	if result := global.DbClient.First(&category, req.CategoryId); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "商品分类不存在")
	}

	var brand db.Brands
	if result := global.DbClient.First(&brand, req.BrandId); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "品牌不存在")
	}

	categoryBrand := db.GoodsCategoryBrand{
		CategoryID: req.CategoryId,
		BrandsId: req.BrandId,
	}

	global.DbClient.Save(&categoryBrand)
...
func (s *GoodsServer) DeleteCategoryBrand(ctx context.Context, req *proto.CategoryBrandRequest) (*emptypb.Empty, error) {
	if result := global.DbClient.Delete(&db.GoodsCategoryBrand{}, req.Id); result.RowsAffected == 0 {
...
func (s *GoodsServer) UpdateCategoryBrand(ctx context.Context, req *proto.CategoryBrandRequest) (*emptypb.Empty, error) {
	var categoryBrand db.GoodsCategoryBrand

	if result := global.DbClient.First(&categoryBrand, req.Id); result.RowsAffected == 0 {
